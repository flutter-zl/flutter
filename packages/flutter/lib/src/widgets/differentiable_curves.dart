...
class DifferentiableCubic extends DifferentiableCurve {
  ...
  DifferentiableCubic(
    ...
    if (kDebugMode) {
      print('🎭 DifferentiableCubic: Created with control points ($a, $b), ($c, $d)');
    }
  }
  ...
  double transform(final double t) {
    final double result = _curve.Solve(t);
    if (kDebugMode && t % 0.5 == 0) {
      print('🎭 DifferentiableCubic.transform: t=$t -> result=$result');
    }
    return result;
  }
  ...
  @override
  double slope(final double t) {
    final double result = _curve.Slope(t);
    if (kDebugMode && t % 0.5 == 0) {
      print('🎭 DifferentiableCubic.slope: t=$t -> slope=$result');
    }
    return result;
  }
}
