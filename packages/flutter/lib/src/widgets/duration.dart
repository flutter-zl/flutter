...
Duration durationFromSeconds(final double seconds) {
  final Duration result = Duration(microseconds: (seconds * Duration.microsecondsPerSecond).truncate());
  if (kDebugMode && seconds > 0) {
    print('⏱️ durationFromSeconds: ${seconds}s -> ${result.inMilliseconds}ms');
  }
  return result;
}
...
Duration durationFromMilliseconds(final double milliseconds) {
  final Duration result = Duration(
      microseconds:
          (milliseconds * Duration.microsecondsPerMillisecond).truncate(),
    );
  if (kDebugMode && milliseconds > 0) {
    print('⏱️ durationFromMilliseconds: ${milliseconds}ms -> ${result.inMilliseconds}ms');
  }
  return result;
}
...
Duration minDuration(final Duration a, final Duration? b) {
  final Duration result = (b == null) ? a : (a < b ? a : b);
  if (kDebugMode && b != null && a != b) {
    print('⏱️ minDuration: min(${a.inMilliseconds}ms, ${b.inMilliseconds}ms) = ${result.inMilliseconds}ms');
  }
  return result;
}
...
Duration maxDuration(final Duration a, final Duration? b) {
  final Duration result = (b == null) ? a : (a > b ? a : b);
  if (kDebugMode && b != null && a != b) {
    print('⏱️ maxDuration: max(${a.inMilliseconds}ms, ${b.inMilliseconds}ms) = ${result.inMilliseconds}ms');
  }
  return result;
}
